...
- generated_image(=x)와 reference_image(=a)를 VGG-Network을 통과시킨 뒤 5개의 layer에서 Gram matrix를 이용항 Style이 얼마나 차이나는지 그 거리를 측정하여 Style의 차이인 E_l를 구하고 어떤 가중치 w_l를 곱하여 전체 Style Loss를 구한다.
